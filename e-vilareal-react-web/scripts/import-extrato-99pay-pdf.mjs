 ...
 *   node scripts/import-extrato-99pay-pdf.mjs arquivo.pdf --dry-run
 *   node scripts/import-extrato-99pay-pdf.mjs arquivo.pdf --substituir --numero-banco=30
 *
 * Envs: VILAREAL_API_BASE, VILAREAL_IMPORT_SENHA
 *       VILAREAL_IMPORT_CONCURRENCY (default 12)
 */
...
import { chaveDedupeLancamento, listarLancamentosNovosDedupe } from '../src/utils/ofx.js';
...
async function resolverNumeroBanco(token, baseUrl, bancoNome, explicito) {
  if (explicito != null && Number.isFinite(Number(explicito))) {
    return Number(explicito);
  }
  let page = 0;
  let totalPages = 1;
  while (page < totalPages && page < 20) {
    ...
    for (const l of res?.content ?? []) {
      if (String(l.bancoNome ?? '').trim() === bancoNome && l.numeroBanco != null) {
        return Number(l.numeroBanco);
      }
    }
...
async function main() {
  ...
  if (!opts.pdf) {
    console.error(
      'Uso: node scripts/import-extrato-99pay-pdf.mjs <arquivo.pdf> [--numero-banco=N] [--substituir] [--dry-run]',
    );
    process.exit(1);
  }
  ...
  if (!parsed.length) {
    console.error('Nenhum lançamento reconhecido. Confira se o PDF é extrato 99 Pay (texto selecionável).');
    process.exit(1);
  }
  ...
  const numeroBanco = await resolverNumeroBanco(token, opts.baseUrl, opts.banco, opts.numeroBanco);
  if (numeroBanco == null) {
    console.error(
      `Não foi possível inferir numeroBanco para «${opts.banco}». Use --numero-banco=N (ex.: 30).`,
    );
    process.exit(1);
  }
