...
export function ModalContatos({ open, onClose, contatos, onChange }) {
  ...
  const abrirAgendamentoWhatsApp = (valorTelefone) => {
    const phone = formatPhoneFromContato(valorTelefone);
    if (!phone) return;
    onClose?.();
    navigate('/whatsapp/agendamentos', { state: { openSchedule: true, phone } });
  };
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <span className="inline-flex items-center gap-1">
                            {item.tipo === 'telefone' && (
                              <button
                                type="button"
                                onClick={() => abrirAgendamentoWhatsApp(item.valor)}
