...
import { buildStateNavegarClienteCobranca } from './cobranca/cobrancaClienteNav.js';
 ...
 */
function BotaoIrCobrancaNoCliente({ codigo, nome, disabled }) {
  const navigate = useNavigate();
  const cod = codigo ? padCliente8Cadastro(codigo) : '';
  if (!cod) return null;
  return (
    <button
      type="button"
      className={botaoSecundario()}
      disabled={disabled}
      onClick={() => navigate('/pessoas', { state: buildStateNavegarClienteCobranca(cod) })}
    >
        ...
        <div className={`max-w-5xl ${isAdmin ? '' : 'mx-auto w-full'}`}>
          ...
          <div className="max-w-xl">
            ...
            <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
              A cobrança automática (.xls) fica na ficha do cliente. Use o botão abaixo no passo 1 do PDF (com
              condomínio selecionado) ou abra um cliente em{' '}
              <button
                type="button"
                className="text-sky-700 dark:text-sky-300 underline hover:no-underline"
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <BotaoIrCobrancaNoCliente
                  codigo={clienteSel?.codigo}
                  nome={clienteSel?.nomeRazao}
                  disabled={!clienteSel}
                />
              </div>
            </div>
          ...
          {step === 4 && importResult && extracao && (
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                <BotaoIrCobrancaNoCliente
                  codigo={extracao?.clienteCodigo ?? clienteSel?.codigo}
                  nome={extracao?.clienteNome ?? clienteSel?.nomeRazao}
                />
              </div>
