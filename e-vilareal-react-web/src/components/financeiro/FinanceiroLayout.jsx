...
function FinanceiroSidebarContent({
  location,
  totalPendentes,
  bancosVisiveis,
  bancosRestantes,
  ...
  return (
    <>
      ...
      <nav className="px-1 space-y-0.5" aria-label="Navegação financeiro">
        <NavLink to="/financeiro" end className={navClass} onClick={onNavigate}>
        ...
        <NavLink to="/financeiro/consolidado" className={navClass} onClick={onNavigate}>
          <BarChart3 className="w-[15px] h-[15px] shrink-0" />
          Consolidado
        </NavLink>
        <NavLink to="/financeiro/total" className={navClass} onClick={onNavigate}>
          <Layers className="w-[15px] h-[15px] shrink-0" />
          Total
        </NavLink>
        <NavLink to="/financeiro/analises" className={navClass} onClick={onNavigate}>
          <Sparkles className="w-[15px] h-[15px] shrink-0" />
          Análises
        </NavLink>
        <NavLink to="/financeiro/investimentos" className={navClass} onClick={onNavigate}>
          <TrendingUp className="w-[15px] h-[15px] shrink-0" />
          Investimentos
        </NavLink>
        <NavLink to="/financeiro/compensacao" className={navClass} onClick={onNavigate}>
      ...
      <p className="px-3 pt-3 pb-0.5 text-[11px] uppercase tracking-wider text-slate-400 font-medium">Bancos</p>
      <div className="px-0 pb-1">
        {bancosVisiveis.map((b) => (
          <BancoItem
            ...
            onClick={wrapNav(() => selecionarBanco(b.numero))}
          />
        ))}
      ...
      <p className="px-3 pt-2 pb-0.5 text-[11px] uppercase tracking-wider text-slate-400 font-medium">Cartões</p>
      <div className="px-0 pb-3">
        <NavLink to="/financeiro/cartoes" end className={cartaoNavClass} onClick={onNavigate}>
          <CreditCard className="w-3 h-3 shrink-0 opacity-60" aria-hidden />
          Resumo fechamentos
        </NavLink>
        <NavLink to="/financeiro/cartoes/fechamentos" className={cartaoNavClass} onClick={onNavigate}>
          <Receipt className="w-3 h-3 shrink-0 opacity-60" aria-hidden />
          Fechamentos fatura
        </NavLink>
        <NavLink to="/financeiro/cartoes/regras" className={cartaoNavClass} onClick={onNavigate}>
    ...
    <div className="flex w-full flex-1 flex-col min-h-[60dvh] lg:min-h-0 lg:h-full overflow-hidden bg-slate-50 dark:bg-slate-950">
      <header className="h-11 shrink-0 flex items-center justify-between gap-2 px-2 sm:px-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <h1 className="text-base font-medium text-slate-900 dark:text-slate-100 truncate">Financeiro</h1>
          {totalPendentes > 0 ? (
            <button
              type="button"
              onClick={() => navigate(pathInboxFinanceiro(location.search))}
              className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300 shrink-0"
            >
              {totalPendentes.toLocaleString('pt-BR')} pend.
            </button>
          ) : null}
        </div>
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          >
            ...
            <span className="hidden sm:inline">Pesquisar valor</span>
          </button>
          <button
            type="button"
            className="inline-flex items-center gap-1.5 text-xs px-2 py-1 sm:px-2.5 sm:py-1 rounded-md border border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800"
            onClick={onOpenImport}
            title={naTelaCartao ? 'Importar fatura' : 'Importar extrato'}
          >
            <Upload className="w-3.5 h-3.5" aria-hidden />
            <span className="hidden sm:inline">{naTelaCartao ? 'Importar fatura' : 'Importar'}</span>
          </button>
          <button
            type="button"
      ...
      <div className="relative flex flex-1 min-h-0 min-w-0 overflow-hidden">
        {mobileNavOpen ? (
          <button
            ...
            aria-label="Fechar menu do financeiro"
        ...
        >
          <FinanceiroSidebarContent {...sidebarProps} onNavigate={() => setMobileNavOpen(false)} />
        </aside>
