...
async function carregarExtratoCompleto({ numeroBanco, clienteId, visaoCliente }, opts = {}) {
  const out = [];
  let page = 0;
  let totalPages = 1;
    ...
    const res = await listarLancamentosExtratoPaginados(
      {
        numeroBanco,
        clienteId,
        visivelCliente: visaoCliente ? true : undefined,
        page,
        size: 500,
...
/**
 * Modo de impressão do acerto (Etapa 5): busca o extrato completo só na hora de imprimir e
 * renderiza no formato do PDF de referência (ordem cronológica com saldo acumulado).
 */
export function AcertoImpressaoModal({ numeroBanco, clienteId, nomeCliente, nomeConta, visaoCliente, onClose }) {
  ...
  useEffect(() => {
    ...
    setLancamentos(null);
    setErro('');
    carregarExtratoCompleto({ numeroBanco, clienteId, visaoCliente }, { signal: ac.signal })
      .then((lista) => setLancamentos(lista))
      .catch((e) => {
        if (e?.name !== 'AbortError') setErro(e?.message || 'Falha ao carregar o extrato completo.');
      });
    return () => ac.abort();
  }, [numeroBanco, clienteId, visaoCliente]);

  const linhas = useMemo(() => {
    if (!lancamentos) return [];
    let saldo = 0;
    return lancamentos.map((l) => {
      const v = valorAssinadoAcerto(l, visaoCliente);
      saldo += v;
      ...
      <div className="max-w-5xl mx-auto p-6 print:p-0 print:max-w-none">
        <div className="flex items-center justify-between gap-2 mb-4 print:hidden">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            Pré-visualização da impressão — {visaoCliente ? 'visão do cliente' : 'visão interna'}
          </p>
        ...
        <header className="mb-3">
          ...
          <p className="text-xs text-slate-500 print:text-black">
            {nomeConta} · emitido em {new Date().toLocaleDateString('pt-BR')} ·{' '}
            ...
            <tbody>
              {linhas.map(({ l, valor, saldo }) => (
                <tr key={l.id} className="border-t border-slate-100 dark:border-slate-800 print:border-slate-300">
                  <td className="px-1 py-0.5 font-mono text-[10px] whitespace-nowrap">
                    {l.numeroLancamento || l.id}
                  </td>
                  <td className="px-1 py-0.5 whitespace-nowrap">{fmtDataAcerto(l.dataLancamento)}</td>
                  <td className="px-1 py-0.5 whitespace-nowrap">{refExibicaoAcerto(l)}</td>
            ...
            {linhas.length > 0 ? (
              <tfoot>
                <tr className="border-t-2 border-slate-400 font-semibold">
                  <td colSpan={4} className="px-1 py-1.5 text-right">
