...
public interface ClienteRepository extends JpaRepository<ClienteEntity, Long> {
    ...
    @Query("SELECT c FROM ClienteEntity c JOIN FETCH c.pessoa LEFT JOIN FETCH c.usuarioResponsavelPadrao WHERE c.codigoCliente = :codigoCliente")
     ...
     */
    @Query("SELECT c FROM ClienteEntity c JOIN FETCH c.pessoa LEFT JOIN FETCH c.usuarioResponsavelPadrao WHERE TRIM(c.codigoCliente) = TRIM(:codigoCliente)")
    Optional<ClienteEntity> findByCodigoClienteFetchPessoaTrim(@Param("codigoCliente") String codigoCliente);

    @Query("SELECT c FROM ClienteEntity c LEFT JOIN FETCH c.usuarioResponsavelPadrao WHERE c.id = :id")
    Optional<ClienteEntity> findByIdFetchResponsavelPadrao(@Param("id") Long id);

    boolean existsByPessoa_Id(Long pessoaId);

    boolean existsByCodigoCliente(String codigoCliente);

    @Query("SELECT c FROM ClienteEntity c JOIN FETCH c.pessoa LEFT JOIN FETCH c.usuarioResponsavelPadrao ORDER BY c.codigoCliente ASC")
