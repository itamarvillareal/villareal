...
class ContratoPartesProcessoResolverTest {
    ...
    void montarPreambuloContratoAluguel_pluralComQualificacoesCompletasEncadeadas() {
        ...
        assertThat(preambulo).contains("como LOCADORA, VRV Soluções Ltda");
    ...
    void montarPreambuloContratoAluguel_removeVirgulaFinalDaQualificacaoDoLocador() {
        ...
        String preambulo = QualificacaoPessoaUtil.montarPreambuloContratoAluguel(locador, locatario, false);

        assertThat(preambulo).contains("como LOCADORA, VRV Soluções Ltda");
        assertThat(preambulo).contains("CEP 75.110-580, e, como LOCATÁRIO");
        assertThat(preambulo.indexOf("VRV Soluções Ltda")).isLessThan(preambulo.indexOf("Marcus Antonio"));
